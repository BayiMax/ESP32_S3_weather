...
MPU_E_A UOld_MPU_Euler_Angle;
MPU_E_A UMPU_Euler_Angle;
...
return:0,正常
    其他,错误*/
uint8_t MPU_Write_Len(uint8_t addr,
                      uint8_t reg, uint8_t len, uint8_t *buf)
{
  int ret;
  ret = I2C_Write_Len(I2C_SLAVE_PORT_NUM, addr, reg, buf, len);
  if (ret != ESP_OK)
  {
...
return:0,正常
    其他,错误*/
uint8_t MPU_Read_Len(uint8_t addr,
                     uint8_t reg, uint8_t len, uint8_t *buf)
{
  int ret;
  ret = I2C_Read_Len(I2C_SLAVE_PORT_NUM, addr, reg, buf, len);
  if (ret != ESP_OK)
  {
    MY_DEBUG("ERROR!%d.", ret);
    return 1;
...
{
  int ret;
  ret = I2C_Write_byte(I2C_SLAVE_PORT_NUM, MPU_ADDR, reg, data);
  if (ret != ESP_OK)
  {
    MY_DEBUG("ERROR!%d.", ret);
    return 1;
...
uint8_t MPU_Read_Byte(uint8_t reg)
{
  uint8_t value;
  int ret;
  ret = I2C_Read_byte(I2C_SLAVE_PORT_NUM, MPU_ADDR, reg, &value);
  if (ret != ESP_OK)
  {
    MY_DEBUG("ERROR!%d.", ret);
    return 1;
...
return:0,ok
    !0,error*/
uint8_t MPU_Get_Accelerometer(short *ax,
                              short *ay, short *az)
{
  uint8_t buf[6], res;
...
int mpu_reg_dump(void)
{
  ...
  for (ii = 0; ii < st.hw->num_reg; ii++)
  {
    if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
      continue;
    if (MPU_Read_Len(st.hw->addr, ii, 1, &data))
      return -1;
    MY_DEBUG("%#5x: %#5x", ii, data);
  }
  return 0;
}
...
int mpu_lp_accel_mode(uint8_t rate)
{
  ...
  if (!rate)
  {
    ...
    tmp[0] = 0;
    tmp[1] = BIT_STBY_XYZG;
    if (MPU_Write_Len(st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp))
      return -1;
    st.chip_cfg.lp_accel_mode = 0;
    return 0;
  }
...
int mpu_reset_fifo(void)
{
  ...
  if (MPU_Write_Len(st.hw->addr, st.reg->int_enable, 1, &data))
    return -1;
  if (MPU_Write_Len(st.hw->addr, st.reg->fifo_en, 1, &data))
    return -1;
  if (MPU_Write_Len(st.hw->addr, st.reg->user_ctrl, 1, &data))
    return -1;
  if (st.chip_cfg.dmp_on)
  {
  ...
  else
  {
    ...
    if (MPU_Write_Len(st.hw->addr, st.reg->int_enable, 1, &data))
      return -1;
    if (MPU_Write_Len(st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
      return -1;
  }
  return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
  ...
  if (st.chip_cfg.gyro_fsr == (data >> 3))
    return 0;
  if (MPU_Write_Len(st.hw->addr, st.reg->gyro_cfg, 1, &data))
    return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
  ...
  else
  {
    ...
    if (rate < 4)
      ...
      rate = 1000;
    data = 1000 / rate - 1;
    if (MPU_Write_Len(st.hw->addr, st.reg->rate_div, 1, &data))
      return -1;
...
int mpu_get_int_status(short *status)
{
  ...
  if (!st.chip_cfg.sensors)
    return -1;
  if (MPU_Read_Len(st.hw->addr, st.reg->dmp_int_status, 2, tmp))
    return -1;
                  ...
                  uint8_t *sensors, uint8_t *more)
{
  ...
  if (st.chip_cfg.fifo_enable & INV_XYZ_ACCEL)
    packet_size += 6;
  if (MPU_Read_Len(st.hw->addr, st.reg->fifo_count_h, 2, data))
    return -1;
  ...
  {
    /*FIFO1/2满,检查溢出*/
    if (MPU_Read_Len(st.hw->addr, st.reg->int_status, 1, data))
      return -1;
  ...
  if (MPU_Read_Len(st.hw->addr, st.reg->fifo_r_w, packet_size, data))
    return -1;
                         ...
                         uint8_t *more)
{
  ...
  if (MPU_Read_Len(st.hw->addr, st.reg->fifo_count_h, 2, tmp))
  {
    MY_DEBUG("??");
    return -1;
  }
  ...
  {
    /*FIFO1/2满,检查溢出*/
    if (MPU_Read_Len(st.hw->addr, st.reg->int_status, 1, tmp))
    {
      MY_DEBUG("??");
      return -1;
    }
    ...
    // {
    // MY_DEBUG("reset fifo:%d.", mpu_reset_fifo());
    // MY_DEBUG("??");
    // return -2;
    // }
  }
  if (MPU_Read_Len(st.hw->addr, st.reg->fifo_r_w, length, data))
  {
    MY_DEBUG("??");
    return -1;
  }
...
int mpu_set_bypass(uint8_t bypass_on)
{
  ...
  if (bypass_on)
  {
    if (MPU_Read_Len(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
      return -1;
    tmp &= ~BIT_AUX_IF_EN;
    if (MPU_Write_Len(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
      return -1;
    ...
    if (MPU_Write_Len(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
      return -1;
  }
  else
  {
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
  ...
  if (MPU_Read_Len(st.hw->addr, 0x0D, 3, tmp))
    return 0x07;
  tmp[0] &= 0x1F;
  tmp[1] &= 0x1F;
  tmp[2] &= 0x1F;
...
static int get_st_biases(long *gyro, long *accel, uint8_t hw_test)
{
  ...
  if (MPU_Write_Len(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
    return -1;
  if (MPU_Write_Len(st.hw->addr, st.reg->i2c_mst, 1, data))
    return -1;
  if (MPU_Write_Len(st.hw->addr, st.reg->user_ctrl, 1, data))
    return -1;
  ...
  for (ii = 0; ii < packet_count; ii++)
  {
    ...
    if (MPU_Read_Len(st.hw->addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
      return -1;
                  ...
                  uint8_t *data)
{
  ...
  if (tmp[1] + length > st.hw->bank_size)
    return -1;
  if (MPU_Write_Len(st.hw->addr, st.reg->bank_sel, 2, tmp))
    return -1;
  if (MPU_Write_Len(st.hw->addr, st.reg->mem_r_w, length, data))
    return -1;
  return 0;
}
                 ...
                 uint8_t *data)
{
  ...
  if (MPU_Write_Len(st.hw->addr, st.reg->bank_sel, 2, tmp))
    return -1;
  if (MPU_Read_Len(st.hw->addr, st.reg->mem_r_w, length, data))
    return -1;
  return 0;
}
...
#define LOAD_CHUNK (16)
  ...
  if (MPU_Write_Len(st.hw->addr, st.reg->prgm_start_h, 2, tmp))
    return -1;
...
thresh  mg/ms.
return:0,ok*/
static int dmp_set_tap_thresh(uint8_t axis, unsigned short thresh)
{
 ...
 */
static int dmp_set_tap_axes(uint8_t axis)
{
  uint8_t tmp = 0;
  if (axis & TAP_X)
    tmp |= 0x30;
 ...
 *  @return     0 if successful.
 */
static int dmp_set_tap_count(uint8_t min_taps)
{
  uint8_t tmp;
 ...
 *  @return      0 if successful.
 */
static int dmp_enable_6x_lp_quat(uint8_t enable)
{
  uint8_t regs[4];
  if (enable)
  {
 ...
 *  @return     0 if successful.
 */
static int dmp_enable_gyro_cal(uint8_t enable)
{
  if (enable)
  {
 ...
 *  @return     0 if successful.
 */
static int dmp_enable_lp_quat(uint8_t enable)
{
  uint8_t regs[4];
  if (enable)
  {
...
//  *  @return     0 if successful.
//  */
// static int dmp_register_tap_cb(void (*func)(uint8_t, uint8_t))
// {
//     dmp.tap_cb = func;
//     return 0;
// }
...
//  *  @return     0 if successful.
//  */
// static int dmp_register_android_orient_cb(void (*func)(uint8_t))
// {
...
uint8_t mpu_dmp_init(void)
{
  uint8_t res = 0;
  //	MPUGPIO_INit();
  MY_DEBUG("mpu_init is ready");
  if (mpu_init() == 0)
  { //初始化MPU6050s
    MY_DEBUG("mpu_init is ok");
    ...
    if (res)
      return 2;
    res = mpu_set_sample_rate(50); //设置采样率
    if (res)
      return 3;
...
uint8_t Get_MPU6050_EA(void)
{
  float pitch, roll, yaw; //欧拉角
  uint8_t x;
  x = mpu_dmp_get_data(&pitch, &roll, &yaw);
  if (x == 0)
  {
    MPU_Value.pitch = pitch;
    MPU_Value.roll = roll;
    MPU_Value.yaw = yaw;
    return 1;
  }
 ...
 * @param pvParameters
 */
void MPU_Test(void *pvParameters)
{
  while (1)
  {
    ...
    if (Get_MPU6050_EA() == 1)
    {
      MPU_Value.Data_Flag = true;

      MPU_Value.Temp = (float)MPU_Get_Temperature() / 100;
      MPU_Get_Gyroscope(&MPU_Value.gx, &MPU_Value.gy, &MPU_Value.gz);
      MPU_Get_Accelerometer(&MPU_Value.ax, &MPU_Value.ay, &MPU_Value.az);

      // MY_DEBUG("...................................................................");
      // MY_DEBUG("Temp:%.2f.", MPU_Value.Temp);
      // MY_DEBUG("ax:%d.", MPU_Value.ax);
      // MY_DEBUG("ay:%d.", MPU_Value.ay);
      // MY_DEBUG("az:%d.", MPU_Value.az);
      // MY_DEBUG("gx:%d.", MPU_Value.gx);
      // MY_DEBUG("gy:%d.", MPU_Value.gy);
      // MY_DEBUG("gz:%d.", MPU_Value.gz);
      // MY_DEBUG("pitch=%.3f", MPU_Value.pitch);
      // MY_DEBUG("roll=%.3f", MPU_Value.roll);
      // MY_DEBUG("yaw=%.3f", MPU_Value.yaw);
      // MY_DEBUG("...................................................................");
    }
