 ...
 * @LastEditTime: 2022-10-20 15:23:08
 * @FilePath: \esps3_V1\components\MPU6050\include\mpu6050.h
 * @Description:
 *
...
AD0接GND,转为读写地址后,为0XD1和0XD0(如果接VCC,为0XD3和0XD2)*/
#define MPU_ADDR 0x68  /*mpu地址*/
#define MPU_READ 0XD1  /*读地址*/
#define MPU_WRITE 0XD0 /*写地址*/
...
extern _MPU_VALUE MPU_Value;

typedef struct _MPU_E_A
{
  float pitch;
  float roll;
  float yaw;
...
extern MPU_E_A UOld_MPU_Euler_Angle;
extern MPU_E_A UMPU_Euler_Angle;

unsigned char Get_MPU6050_EA(void);
...
int mpu_lp_motion_interrupt(unsigned short int thresh, unsigned char time,
...
int mpu_get_lpf(unsigned short int *lpf);
int mpu_set_lpf(unsigned short int lpf);

int mpu_get_gyro_fsr(unsigned short int *fsr);
int mpu_set_gyro_fsr(unsigned short int fsr);

int mpu_get_accel_fsr(unsigned char *fsr);
int mpu_set_accel_fsr(unsigned char fsr);

int mpu_get_compass_fsr(unsigned short int *fsr);

int mpu_get_gyro_sens(float *sens);
int mpu_get_accel_sens(unsigned short int *sens);

int mpu_get_sample_rate(unsigned short int *rate);
int mpu_set_sample_rate(unsigned short int rate);
int mpu_get_compass_sample_rate(unsigned short int *rate);
int mpu_set_compass_sample_rate(unsigned short int rate);
...
int mpu_read_fifo_stream(unsigned short int length, unsigned char *data,
                         unsigned char *more);
int mpu_reset_fifo(void);

int mpu_write_mem(unsigned short int mem_addr, unsigned short int length,
                  unsigned char *data);
int mpu_read_mem(unsigned short int mem_addr, unsigned short int length,
                 unsigned char *data);
int mpu_load_firmware(unsigned short int length, const unsigned char *firmware,
                      unsigned short int start_addr, unsigned short int sample_rate);
...
unsigned short int inv_row_2_scale(const signed char *row);
unsigned short int inv_orientation_matrix_to_scalar(const signed char *mtx);
